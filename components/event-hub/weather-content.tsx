...
function TemperatureRow({ high, low, eventTemperature }: Pick<EventForecast, "high" | "low" | "eventTemperature">) {
  return <dl className="mt-5 grid grid-cols-3 gap-2">
    {[{ label: "High", value: high }, { label: "Low", value: low }, { label: `Around ${OYSTER_ROAST_EVENT.timeLabel}`, value: eventTemperature }].map(({ label, value }) => (
...
function TypicalConditions({ typical }: { typical: TypicalWeather | null | undefined }) {
  ...
  return <div>
    ...
    <p className={`mt-1 ${detailClass}`}>A little context for November, not a forecast.</p>
    {typical ? <>
      <TemperatureRow {...typical} />
    ...
    </> : <p role="status" className={`mt-4 ${detailClass}`}>{typical === undefined ? "Looking back at past Novembers…" : "Historical weather is taking a quick break. Check back shortly."}</p>}
  </div>;
}

function ForecastConditions({ weather }: { weather: EventWeather & { forecast: EventForecast } }) {
  ...
  return <div>
    ...
    <p className={`mt-1 ${detailClass}`}>{eventDay ? "Event-day forecast" : OYSTER_ROAST_EVENT.shortDateLabel} · {forecast.condition}</p>
    {eventDay && forecast.hours.length ? <ol aria-label="Hourly event forecast" className="mt-4 grid grid-cols-4 gap-2 sm:grid-cols-7">
      {forecast.hours.map((hour) => <li key={hour.at} className={`min-w-0 rounded-xl px-1.5 py-3 text-center ${hour.at === Date.parse(OYSTER_ROAST_EVENT.startsAtUtc) ? "bg-[#355f9e]/12 ring-1 ring-inset ring-[#355f9e]/20" : "bg-[#355f9e]/5"}`}>
...
export function WeatherContent({ weather, typical, loading = false }: { weather: EventWeather | null; typical: TypicalWeather | null | undefined; loading?: boolean }) {
  return <>
    <div className="py-5">
      {loading && !weather ? <p role="status" className={detailClass}>Checking the skies in St. Johns…</p> : weather?.forecast ? <ForecastConditions weather={{ ...weather, forecast: weather.forecast }} /> : <>
        <TypicalConditions typical={typical} />
    ...
    <div className="mt-5 border-t border-[#202523]/10 pt-4">
      ...
      {weather?.current ? <>
        ...
        <p className={`mt-1 ${detailClass}`}>{OYSTER_ROAST_EVENT.cityLabel} · {weatherTime(weather.current.at, weather.timeZone, true)} local time · model estimate, not the event forecast.</p>
